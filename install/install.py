...
print "kylin_ops_dir IS====>:",kylin_ops_dir
...
class PreSetup(object):
    def __init__(self):
        ...
        self.mail_port = 25
        self.mail_addr = 'test_kylin@wanxue.cn'
        self.mail_pass = ''
        self.ip = ''
    ...
    def write_conf(self, conf_file=os.path.join(kylin_ops_dir, 'kylin_ops.conf')):
    ...
    def _require_pip(self):
        ...
        self.check_bash_return(ret_code, "安装kylin_ops 依赖的python库失败！")
    ...
    def _input_mysql(self):
        while True:
            ...
            if mysql != 'n':
                ...
                db_user = raw_input('请输入数据库服务器用户 [kylin_ops]: ').strip()
                db_pass = raw_input('请输入数据库服务器密码: ').strip()
                db = raw_input('请输入使用的数据库 [kylin_ops]: ').strip()
    ...
    def start(self):
        color_print('请务必先查看wiki https://github.com/cwm-kylin/kylin_ops/wiki')
        time.sleep(3)
        self.check_platform()
        self._rpm_repo()
        self._depend_rpm()
        ...
        self._input_smtp()
        self.write_conf()
        os.system('python %s' % os.path.join(kylin_ops_dir, 'install/next.py'))
