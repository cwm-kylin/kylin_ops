...
from usermanagers.models import AdminGroup
from kylin_ops.api import *
from kylin_ops.settings import BASE_DIR, EMAIL_HOST_USER as MAIL_FROM
...
def server_add_user(username, ssh_key_pwd=''):
    """
    add a system user in kylin_ops
    在kylin_ops服务器上添加一个用户
    """
...
def user_add_mail(user, kwargs):
    ...
    mail_title = u'恭喜你的跳板机用户 %s 添加成功 kylin_ops' % user.name
    mail_msg = u"""
    Hi, %s
        您的用户名： %s
        您的权限： %s
        您的web登录密码： %s
        您的ssh密钥文件密码： %s
        密钥下载地址： %s/usermanagers/key/down/?uuid=%s
...
def server_del_user(username):
    """
    delete a user from kylin_ops linux system
    删除系统上的某用户
    """
...
def get_display_msg(user, password='', ssh_key_pwd='', send_mail_need=False):
    if send_mail_need:
        ...
        密码：%s <br />
        密钥密码：%s <br />
        密钥下载url: %s/usermanagers/key/down/?uuid=%s <br />
        该账号密码可以登陆web和跳板机。
