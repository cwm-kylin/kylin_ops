...
from permission.perm_api import user_have_perm
from django.http import HttpResponseNotFound
from kylin_log.log_api import renderJSON

from kylin_log.models import Log, ExecLog, FileLog, TermLog
from kylin_ops.settings import LOG_DIR
import zipfile
import json
import pyte
...
def log_list(request, offset):
    ...
    return render_to_response('kylin_log/log_%s.html' % offset, locals(), context_instance=RequestContext(request))


@require_role('admin')
def log_detail(request):
    return my_render('kylin_log/exec_detail.html', locals(), request)
...
def log_kill(request):
    ...
    if log:
        ...
        return render_to_response('kylin_log/log_offline.html', locals(), context_instance=RequestContext(request))
    else:
...
def log_record(request):
    ...
    if request.method == "GET":
        return render(request, 'kylin_log/record.html')
...
def log_detail(request, offset):
    ...
    if offset == 'exec':
        ...
        try:
            ...
            result = {}
        return my_render('kylin_log/file_detail.html', locals(), request)
